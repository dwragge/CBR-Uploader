...
$.getScript('http://mozilla.github.io/pdf.js/build/pdf.js', function() {
    ...
    function cleanText(text) {
        ...
        text = text.substring(startIndex + 91, endIndex)
    ...
    function parseProdcuts(pdfText) {
        var text = cleanText(pdfText);
        var regex = /(?:\d YRS)?\s\d+\sF&P\s(\w+).+?(?=\d\s@)(\d)\s@\s([0-9,.]+)\s[0-9.]+\s[0-9.,]+((?:\sDiscount : [0-9.,]+-)*)/g
    ...
    function getDate(text) {
        var regex = /Date:\s+((?:[0-9]{1,2}\/?){3})/
        var match = regex.exec(text)
        var dateSplit = match[1].split('/')
        if (dateSplit[0].startsWith('0')) {
            dateSplit[0] = dateSplit[0].substr(1)
        }
